...
const AddLectureModal = ({ courseId, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`${API_BASE_URL}/lectures/`, formData, {
        headers: {
      ...
      if (response.data.success) {
        ...
        setError(response.data.message || "Failed to create lecture");
      }
    } catch (err) {
        ...
        <div className="sticky top-0 bg-gradient-to-r from-green-600 to-emerald-600 text-white px-4 py-3 rounded-t-xl">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-lg font-bold">Add New Lecture</h3>
            </div>
            <button
              onClick={onClose}
              disabled={loading}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex items-center gap-2 p-3 bg-blue-50 border border-blue-200 rounded-lg">
              <input
                ...
                disabled={loading}
              />
              <label htmlFor="is_preview" className="flex-1 cursor-pointer">
