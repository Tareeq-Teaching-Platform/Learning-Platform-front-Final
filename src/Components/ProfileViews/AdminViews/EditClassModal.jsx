...
const EditClassModal = ({ onClose, classData }) => {
  ...
  const updateLevelMutation = useMutation({
    mutationFn: async (data) => {
      ...
      const response = await axios.put(
        `${API_BASE_URL}/levels/${classData.id}`,
        data,
        {
          headers: {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    updateLevelMutation.mutate(formData);
  };
          ...
          {updateLevelMutation.isError && (
            <div className="mb-4 p-3 bg-red-50 border border-red-400 rounded-lg flex items-start gap-2">
              ...
              <span className="text-red-700 text-xs">
                {updateLevelMutation.error?.response?.data?.message ||
