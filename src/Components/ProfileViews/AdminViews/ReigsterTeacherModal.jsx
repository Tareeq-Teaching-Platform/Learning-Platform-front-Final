...
const RegisterTeacherModal = ({ onClose }) => {
  ...
  // useMutation for registering teacher
  const registerTeacherMutation = useMutation({
    ...
    onError: (error) => {
      toast.error(
        error.response?.data?.message || "Failed to register teacher"
      );
    },
  });
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    registerTeacherMutation.mutate(formData);
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Body */}
        <div className="p-4">
          {registerTeacherMutation.isError && (
            <div className="mb-4 p-3 bg-red-50 border border-red-400 rounded-lg flex items-start gap-2">
              ...
              <span className="text-red-700 text-xs">
                {registerTeacherMutation.error?.response?.data?.message ||
